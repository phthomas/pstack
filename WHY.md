...
**Start from a brain dump, because that's the honest input.** I don't think in clean specs. I think in a messy pile of "wouldn't it be cool if," half-remembered constraints, and stack preferences. Pretending otherwise just adds a step where I procrastinate. So the front door is `dump.md` — say everything, badly — and `/ps-start` exists only to turn that mess into something an agent can execute: gate it, interview me on the gaps, sort it into specs. The questions that remain get asked when they matter — at the phase that needs them — not as homework up front.

**The same machinery works on code that already exists.** Not everything starts from nothing. Sometimes the code is already there and the ambiguity just points the other way — not *what should this be*, but *what is this, and where is it going*. The why, where to look, the conventions it already follows: those live in someone's head, not the repo. So brownfield gets the same front door — dump what you know, and the same `/ps-start` reads the code first, asks only what the code can't answer, and writes down the shape, the rules, and the forward plan nobody wrote down. Greenfield or brownfield, the job is the same: make the implicit explicit before building.
