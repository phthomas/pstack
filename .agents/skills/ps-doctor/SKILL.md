...
- **Model tiers** — can this harness pick a model per spawned agent (the Task tool's model field on Claude Code; per-agent model config on OpenCode)? Note the reachable rungs — a frontier model, a mid one, a small one — and whether a per-spawn reasoning-effort knob exists (rare; without one, spawned agents inherit the session's thinking and the packs' output discipline is the control). No per-spawn selection = a `model-tiers` line is inert and every agent runs on the session model; say so. Verification pointer: `scripts/agent-models.sh` (pstack repo) reads Claude Code's own transcripts and prints which model actually ran each spawned agent — the ground truth behind the manifest lines (Claude Code only; other harnesses don't write those transcripts, so there the manifest is the pointer).
...
## Sync the map
Offer to write or update the `## Capabilities` section in CLAUDE.md to match what was found — capability -> provider -> fallback, one line each — including the `model-tiers` line (deep = session model · standard · mechanical — the latter two usually sharing the rung one below) when per-spawn selection exists, with suggested defaults from the reachable rungs. That section is the single place providers are named; every other skill resolves against it, so keep it true rather than aspirational. (No CLAUDE.md yet? /ps-start generates one with this section built in.)
