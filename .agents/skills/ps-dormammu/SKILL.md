...
2. Resolve capabilities: read CLAUDE.md `## Capabilities` (probe as /ps-doctor does if it's missing or stale). Note what this run has — reviewers, parsimony, docs, browser, gate tools — and what degrades; it becomes each phase's manifest line.
   If a `model-tiers` line is mapped and this harness sets models per spawn (the Task tool's model field on Claude Code), assign by the nature of the task, never the name of the role: builders run **standard** — **deep** when the spec says `Complexity: hard`, the work is design- or prose-craft-heavy, or a security surface is in play; judges run on the session model; fix application and delta re-checks run **mechanical**. Misclassification is cheap by design: an agent that fails or parks retries ONCE on the next distinct tier up before counting as failed, and the gate re-runs after mechanical work regardless of tier. Reasoning effort inherits from the session on most harnesses — the pack's output-discipline block is the real control; where a per-spawn effort knob exists, tier it too (deep inherits the session's, mechanical runs low). No per-spawn selection? Everything runs on the session model — the manifest says so.
...
Every pack — builder, judge, or fixer — ends with the same **output discipline** block; it is what keeps agents alive on prose-heavy phases: work file-by-file; never restate or quote a whole file in prose or in thinking; draft deliverables in the file with incremental Writes and Edits, not in your head first; emit a tool call early and often, with at most a sentence or two of text between calls. An agent that still dies on an output-token overflow gets RESUMED with these rules and smaller work units — one file, one finding at a time (SendMessage on Claude Code; its context is still warm) — not respawned from scratch to re-buy everything it had read.
...
5. Panel — yours, not the builder's (a spawned agent can't spawn judges; run it when the builder reports green): /ps-review on the phase diff at its default weight — one combined fresh-context judge — splitting into the full panel only on /ps-review's triggers (security surface, multi-surface diff, `Review: full` in the spec). Must-fixes go back to the BUILDER: resume it with the finding list — it still holds the phase's context — or apply the trivially mechanical ones yourself; a fresh fix agent is the last resort (mechanical tier, discipline block, small work units). Re-gate, re-review the deltas — at most 3 cycles, then park the phase with its findings flagged. Arbitration is /ps-review's rule: the spec is the objective function; parsimony wins ties.
...
- A short report over every phase attempted: what got built; which criteria pass and which don't; what the gate caught; what the panel applied vs flagged — including anything parked at the review cap or blocked by another run, with its findings; every assumption made unattended (suggest /ps-adr where it matters); known issues and risks; and the phase's manifest line — which judges ran with which providers on which model tier, what degraded and why. Plus two lines for the run: the waves as executed, parallel or not; and any agent deaths and resumes (what overflowed, on which step) so the packs can be tuned.
- Everything on the run branch, unmerged — and /ps-close's review NOT pre-run: the per-phase panels are this run's review ceiling; the close panel happens attended. Then it's my turn: read the report, deep-QA the flags, and /ps-close to ship — or /ps-spec more clarity into a phase and run you again.
