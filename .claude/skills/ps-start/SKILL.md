...
- CLAUDE.md <- stack, test framework, architecture, conventions, standing rules — plus a `## Capabilities` section: probe the environment the way /ps-doctor does and map capability -> provider -> fallback, one line each (correctness review, parsimony review, product review, docs lookup, browser, gate tools, canvas, craft — and `model-tiers` when the harness picks models per spawn: deep = session model, standard and mechanical usually one rung below). Write what's actually there, not what should be. Lean — about a page. Capabilities live in the environment, never in the core: this section is the one place providers are named; every other skill resolves against it and degrades gracefully. Borrowed craft skills get their pstack adaptations here too, as standing rules (e.g. prototype captures land in the spec and an ADR, not an issue tracker) — adapt in this layer, never by editing provider files.
- CONTEXT.md <- the project glossary (ubiquitous language), only once domain vocabulary actually emerges in the interview: each term defined in a line or two, with `_Avoid_:` synonym bans so the whole system converges on one word per concept. Append as terms appear; never pad it. Craft skills that read CONTEXT.md pick it up for free.
- ROADMAP.md <- the index: a phases-at-a-glance table (status, in-MVP, spec link per phase) plus the MVP boundary.
- specs/NN-name.md <- one per phase, shaped like specs/_TEMPLATE.md: Goal, Coordination (optional), Requirements, Acceptance criteria, Performance budget (optional), Hardstop, Open questions, Out of scope. Spec EVERY phase, not just the first. UI-facing criteria name their Playwright check the way backend criteria name their pytest. Where something stays genuinely undecided, leave an [OPEN: ...] marker rather than guessing — then tell me the count, and that the build skills will ask when each phase starts. A question that a throwaway prototype could answer better than I can — "does this state model feel right?", "which of these UI shapes?" — gets [OPEN-SPIKE: ...] instead: the builder answers it with the prototype craft skill when that capability exists, rather than asking me.
